...
<details>
<summary>Moving Logic To Components That Actually Need It ("Moving State Down")</summary>

So now that we added this new Question component, we fixed this problem with our key, but we also kind of added a new problem, because now we have to pass a lot of props that are managed in the Quiz component to the Question component just to make sure that it works correctly. And this is something we can improve here, and at the same time, we'll improve this Quiz component, because, for example, all that logic where we manage the answerState, why does this need to happen here in the Quiz component? Well the answer is, it doesn't.

That is a great example for a state that does not need to be lifted up. Because we can simply go to the Question component and then there, add a new state with the useState hook that must be imported, of course, that will belong only to this Question component. And this could be a state where we wanna store an object, where we have the selectedAnswer, let's say, which initially is an empty string, and then let's say, isCorrect property, which initially is null, but which could then be true or false. This is some state we could manage here and I'll name it answer and have my setAnswer state updating function.

And the idea now simply is to have a function, handleSelectAnswer, that should be triggered whenever an answer is selected here. So therefore I'll point at handleSelectAnswer down there, and I then expect to get my answer text here. We can then call setAnswer and set this object to a new object where selectedAnswer is equal to the answer we received here and isCorrect now must be derived. But it should only be derived after this one second which we actually spend here in the Quiz component. So for the moment, I'll still set it to null here, which basically should say that we don't know yet whether it's correct or wrong.

So therefore here, I then wanna set a timer with setTimeout where after one second we do set isCorrect to true or false. So where I call setAnswer again and where I now set isCorrect equal to, and then I wanna set this to true if this is the right answer or false if it's not. But for that, we, of course, need to know what the right answer is.

Now the great thing about this project is that we have all questions and answers in this questions.js file. So we can simply import this file into Question.jsx to take a look at the current question and get the right answer. So here in this file we can also import QUESTIONS from and then going up one level, questions.js. So that now, we can take a look at those questions here.

And the great thing is that we also, of course, can find out what the index of the currently active question is because in the Quiz component, we're setting this index as a value on key on this Question component. And we can, of course, try using that key on our own component for our own purposes as well. So in our Question component, we can therefore destructure this key prop and also use it here.

So now with that here in this Timeout, we can access QUESTIONS for the key, so for this activeQuestionIndex to take a look at a specific question. And then we take a look at the first answer there, which we know is the correct answer for the selected question. And we compare that to our answer here. And if that here yields true, we know that the correct answer was picked, otherwise it's the wrong answer.

And with that, we can now use our own answerState to update the UI. So the value for answerState here should now be correct or wrong. So we can add a little helper variable here, which I'll name answerState, which initially is simply an empty string, just as my old answerState here was in the Quiz component. But if in our answerState here in the Question component, if there we got a selectedAnswer, I wanna set answerState depending on answer.isCorrect. And if that's true, I wanna set it to correct, otherwise I'll set it to wrong. And then it's this answerState that will be passed to answers.

Now of course, that's now clashing with my answerState prop, but this prop can and should now be removed because we don't need it anymore. Now onSelectAnswer will not be removed though because we will still need to pass the selectedAnswer to the Quiz component, because, of course, in the Quiz component, we still wanna store all the selected userAnswers so that we can also find out when the quiz is over.

Therefore, I'll call onSelectAnswer here once this Timeout expired and once we show the user whether the picked answer is correct or not. To be precise, here, I then wanna set another timer again so that we don't instantly move away, where after two seconds, we call onSelectAnswer and pass the selectedAnswer to this function, and that will then trigger the connected function in the Quiz component.

Now with that, we can still get rid of some props here in this Question component though. For example, answers and questionText, because we are importing the raw data now and we know that the key is the activeQuestionIndex. Therefore, we can get rid of those props here and instead, here, simply use questions for the key and then text, because the key, as mentioned, is the activeQuestionIndex. And for the answer, it's QUESTIONS[key].answers. So with that, we got two props less that we have to accept.

The selectedAnswer is also not needed anymore because we're now storing this here in our state that belongs to this Question component. We now just have to update the selectedAnswer down there and set this equal to answer.selectedAnswer, which is that selectedAnswer we are now managing in our own state here in this component.

And with that, this question component now contains more logic and needs less props. Therefore, in the Quiz component, we can and should now remove all those redundant props. To be precise, selectedAnswer, answerState, answers and questionText, and get rid of all the logic that we don't need anymore.

Specifically, we don't need this answerState anymore, and we can, again, derive the activeQuestionIndex just based on the userAnswers.length because this will now only be updated from inside the Question component once we did log in an answer and show the user for two seconds whether that answer was correct or not.

And we don't have to update the answerState here anymore and we should remove those timers here in the Quiz component. Therefore, useCallback here also no longer needs the activeQuestionIndex. And with that, the Quiz component function got leaner again, which is great, and the Question component got more powerful.

Now if we save all that, we'll see that we're now facing an error though. And the error is, if we open the DevTools, that we are using the key prop. This is a prop that's exclusively used by React and must not be used by us. Therefore, we should pick our own prop, for example, questionIndex or just index, any name of your choice. And we should use this prop instead of key in all the places where I did use key in the Question component.

And in the Quiz component, we should now also set this index prop here even though it contains the same value as the key prop. But since this is a prop exclusively reserved for React, we must use our own prop here.

But with that, if we reload, we see our questionnaire, we can log in questions, and of course, we also see that now I instantly get the feedback that tells me whether the question is right or wrong. And we also seem to move through questions pretty quickly now.

Now to make sure that we don't instantly mark an answer as right or wrong, we should actually go here to the answerState in our Question component and not always set it to just correct or wrong, but instead here I wanna check whether answer.selectedAnswer is truthy and answer.isCorrect is not equal to null, because if isCorrect is null, that means that we might have a selectedAnswer, but we don't wanna show the result yet, because we deliberately set isCorrect to null here when we log in an answer.

Now to make sure that we mark the selectedAnswer as selected, we also should make sure that if this condition here is not met, but if we have a selectedAnswer, we set the answerState equal to answered, which is then the condition we're checking for here in the Answers.jsx file to mark an answer as selected.
...
<details>
<summary>Setting Different Timers Based On The Selected Answer</summary>

Now, this quiz application is slowly but steadily taking shape here. We're able to answer those questions and see whether we did pick the right or wrong answer. And we got this nice timer here that tells us how much time is remaining.

At the moment, we'll face a problem though. If I pick an answer a little bit too late, well then this timer up here might actually expire before I learn whether it's right or wrong. We simply might move along too quickly and as you actually maybe saw here, we then also seem to run into some problems where we got multiple timers going on at the same time.

Because whenever we pick an answer, we are of course also setting a timer here in the question component. We're actually setting two nested timers here. And once this inner timer expired, which happens after we marked an answer as right or wrong, we forward the selected answer to the parent component, which is the quiz component here. But that of course means that if this timer here expires before the right or wrong answer is forwarded to the parent component, we switch to a different question because the timer expired. And then again, because now the actually selected answer was forwarded to the parent component.

And in the quiz component, we got these two different situations where we do switch to a different question because we stored a selected answer. One way of moving to the next question and storing an answer is if an answer was skipped because the timer expired. The other way is of course, that we received an answer from the user because an answer was selected. And now both things happen if we select an answer too late.

So what we need to do to avoid this problem is in this question component, we must up update this timer if the user did select an answer, so that we start a new timer, so to say, that only expires after we showed the user the right or wrong answer. And if that timer then expires, I also don't wanna tell the quiz component that the question was not answered, because that will then not be the case.

So to update the timer here, once an answer was chosen, we can add a new computed value here in this question component, which we could name timer. And that should be 10,000 milliseconds or whatever your default maximum amount of time per question should be as one possible value when this question component gets rendered.

But when this component gets re-rendered because an answer was selected, I want to change that value. So, I'll actually set this to a variable here and now change it depending on certain conditions. Because if on this answer state we have here, if the selected answer is truthy, so if we have a selected answer, I want to set the timer to 1000 milliseconds because I'll reveal the correct or right answer after those 1000 milliseconds.

If then thereafter, answer is correct is not equal to null. So if we have the information, whether it is correct or not, I want to set the timer to 2000 milliseconds because that's then the time it'll take until we move to the next question. And of course, 1000 and 2000 therefore are the same values I have down there for my timers.

And with that, we can and should use those computed timer values as values for the timeout prop on the question timer. And therefore, whenever timer changes inside of the question timer component, a new interval and timeout will be started and the progress will be updated accordingly. Because timeout is received as a prop here and used as a dependency on our timeout here.

Now in addition, I'll also accept a mode prop in this question timer component because I wanna set this mode prop here as a class name simply for styling purposes, therefore. Because I prepared the index CSS file such that if we set answered answered as a mode here, the progress bar will receive a different color, which I simply want to use to show the user that something changed. And therefore, in the question component on question timer, I'll now set this mode prop to my answer state, since that will be answered or correct or wrong.

With that, if we save this, if I reload, if I pick an answer, you see the color changed, but we still seem to be facing some problems here. For example, I still switched to a different question thereafter unexpectedly, and the color of the progress bar changes, but you also see the progress bar does not update here. So, there seem to be some problems here.

And one problem simply is that we don't reset the interval whenever the timeout changes. And therefore this interval is not in line with the new timer and the new max value of the progress bar. The easiest way of fixing this, is to again add a key here to question timer and to set this equal to timer, so that when we change the timer value, we destroy and recreate the question timer component and force the interval to be recreated, therefore.

Now, the other issue was that we still skipped a question thereafter and that happens because whenever this timer expires in there, we still trigger this on skip answer function. But I only want to trigger this function if no answer was selected. Therefore, here in the question component, we can for example, go down here and check if answer, selected answer is equal to an empty string, which means no answer has been selected. In which case the value for on timeout should indeed be on skip answer. Otherwise, here I want to have null. I don't want to have a value for on timeout, because I don't want to execute any function in that case.

If I save this and reload, now I can log in a question, we got new timers expiring and we then move on to the next question and we're now not skipping questions unexpectedly.

And now with all of that out of the way, we got a working question component. We can move through those questions, skip questions, give the user some nice feedback regarding which question was selected and whether it's right or wrong. And therefore, the next step now is to make sure that once we moved through all those questions, that we also update that summary screen or that we show some summary screen at the end where we basically just summarize the results and where we tell the user how good or bad they performed in this quiz.

So that's what we'll work on next.

Here are the full codes of this section 
...
The display will get the mark if correct or wrong answer

![alt text](image-11.png)
</details>

<details>
<summary>Outputting Quiz Results</summary>
...
Now next to this div with the summary stats I then want to have an ordered list where I output more details about all those questions. So that here we in the end output list items where every list item will have an H3 element where I wanna output the number of the question, for example two, then a paragraph where I output the question text. And this should receive a class name of question for styling purposes. So here we'll have the question text then, and then another paragraph with a class name of user-answer, where I wanna output the user's answer. And of course again this will all be replaced with actual data soon. But therefore we'll need to collect that data.

And of course we're already doing that to some extent, in the quiz JSX file we are storing all these selected answers, and that is of course the most important piece of information, because it's this list of selected answers that can then be used to calculate all these numbers and output these details here.

So in the end, it's in the quiz component file where we want to use the summary component file we just worked on. And therefore here we should import summary from summary JSX, and then output this summary component here if the quiz is complete, if it's over. And we can then pass the user answers as a prop to that summary component. So here it's this user answers state value which I want to provide as a value for this user answers prop to summary. Of course you could use a different prop name here, but I'll stick to user answers.

And in the summary component we can therefore destructure the props object to get hold of this user answers object. And now we can use this user answers object to derive all the different data points we need in this component. For example, we can output those answers and questions down here. Now for that, we'll also need the questions of course, so we should import questions from, and then going up one level questions.js, so that we have the raw question data available as well.

With that imported down here in this ordered list we can map through the user answers to output a list of items based on those user answers. And here I then wanna return this list item for every answer, and I therefore wanna output the answer here, and we should add a key here, which also can be the answer, since this will be a unique value across all answers, we can output the number of the question to which this answer belongs, with help of that index which we also get automatically in this function that we pass to map, that will be provided automatically by JavaScript. And that index of course starts at zero, so we should output index plus one here, so that we show the number one for the first question, the number two for the second question, and so on.

And to output the question text here we can now use the questions raw data and use this index to access the different questions to which the different answers belong, and then output the text of that question here. With that if we save that, you see we're getting that list of questions on this screen, and we also see which answers were picked for every question.

Now it is of course possible that a user skips a question though, if they don't pick an answer before the timer expired. Therefore, instead of always outputting answer down there which could be null if no answer was picked, because in that case in the quiz component, we are storing null as a answer if the timer expired and no answer was picked. So therefore, instead of outputting null down there, we should check if answer is truthy and as a fallback value if it's not, so if no answer was picked, we could output skipped here.

In addition, I wanna set this class name here conditionally to show the user on this screen here whether an answer is correct or not. To do that I'll dynamically derive the CSS class that should be added here, for the answer we're currently looking at here. And I'll add a CSS class variable for that which is always user-answer. So that's the base CSS class that should always be added.

But then here we can check if answer is equal to null which means it has been skipped, in that case, I want to add something to that string. I want to add a white space and then thereafter the skipped CSS class to apply a special style if this answer was skipped. Else if we have an answer, I wanna check if it's correct. So then I wanna check if answer is equal to questions for the current index, and then there we know it's the first answer that's the right answer. So if this here yields true, we know that the right answer was picked. And in that case, I want to add the correct CSS class here. And this white space here is important by the way, to make sure that as we're concatenating to this string here, we're separating those CSS classes with white spaces. And then last but not least, in the last else case here we know that an answer was picked but that it's the wrong answer, and therefore here I want to add the wrong CSS class. And this here should be an else if branch by the way, I forgot the if keyword. And now it's this CSS class that should be added down there.

And with all that done, you now see that incorrect answers have a reddish text and right answers, correct answers, have a greenish text.

Now we also need the summary statistics up here. And for that of course we need to take a look at all answers and calculate the share of skipped, wrong and right answers. So here I'll start by calculating the share of skipped answers, by taking a look at my user answers and then filtering for all the answers where the answer is equal to null. Because as I mentioned before, we add null to this answers array if an answer is skipped. So with that, we get the skipped answers.

The correct answers, are all the user answers where the answer is equal to the first answer in the raw data. So therefore here we actually also need the index of that answer in that answers array, so that we can select a correct question in that questions raw data array. And then we compare the answer to the first answer in that raw data. And if we have equality, we know that it's a correct answer and it will be added to this correct answers array.

And therefore we can, for example calculate the share of skipped answers, so skipped answers share, by taking the length of the skipped answers array so the number of skipped answers we have, and dividing that by the overall answers length, so the number of answers we got in total. We can then multiply this with 100, and then use math round to round that overall result.

And we can of course apply the same logic for the correct answers. So the correct answers share is simply the correct answers length divided by the user answers length. Well and the wrong answers share is then of course simply 100 minus the skipped answers share minus the correct answers share. So whatever is left must be our wrong answers share.

And with that we can output the skipped answers share here. And I'm keeping this percentage sign thereafter so that we still see that as text on the screen. Here we then have the correct answers share. And here, for the last summary statistics number, we have the wrong number share.

If we save that, we see these numbers here. Now to check that it all really works as it should, I'll restart the quiz, and I'll let the first question expire here without picking answer, then we can try to answer the next one. Looks like this here is wrong. Maybe try to pick a right answer for the one thereafter. And then I'll again let the next question thereafter expire, really just to see whether this then all behaves correctly and looks right on the summary screen. Here we can again pick an answer. Looks like it was wrong, maybe you can do better. Take this answer here, and here for the last question, I'll also pick a wrong answer.

And with that here, you see I'm back on this screen and this is all looking good to me. I got these different questions, I skipped two questions. I also got two questions right and therefore it makes sense that we have the same percentages here. And then we got the incorrectly answered percentage where I got three answers wrong. So that's looking good here, but if we open the developer tools, I'm getting an error here, that I got two children with the same key.

And the problem here simply is that of course I got two skipped answers, and since I'm using my answer as a key, I got twice the key null being used here. Therefore, here's a situation where the index is a better key. And typically you wanna avoid using this index of the map method, because this is now not tied to the data but instead by the position of the data in the array. So if two items in the array would swap position the data would be changed, but the index would not change because the position one still has the index zero, for example. But here it's fine because we aren't swapping any answers in this list, we're not swapping data here, and index indeed is the the best key we have here.

Therefore, here, I'll change this. And with that, the error is still there from before but we would now not get it again if we would go through the quiz again. And that's therefore, this quiz finished.

Full codes of **Summary.jsx** component :
